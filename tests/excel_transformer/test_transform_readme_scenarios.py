...
def test_readme_scenario_3_nested(tmp_path: Path):
    excel_path = REPO_ROOT / "tests" / "data" / "sample.xlsx"

    # Scenario 3 config from README (nested objects under [out])
    cfg_text = (
        """
        [map]
        {
    ...
    cfg_path = tmp_path / "config3.conf"
    _write_config(cfg_path, cfg_text)
